...
YoParser::YoParser(YoSystem * s, const char * input)
{
	system = s;
	init(input);
}

YoParser::YoParser(YoSystem * s, const char * input, int len)
{
	system = s;
	init(input, len);
}
...
void YoParser::dumpErrorLine(const YoParserToken& token)
{
	...
	dumpErrorLine(token.line, pos, token.len);
}

void YoParser::dumpErrorLine(int line, int linePos, int tokenLen)
{
	int errorLine = line;
...
void YoParser::dumpError()
{
	...
	printf("[err-parser-%d]: %s\n", (int)error, errorMsg.c_str());
	dumpErrorLine(errorLine, errorLinePos, errorLoc.token.len);
}
...
static void yoParserDumpNode(YoParserNode * node, int depth, EYoParserNodeScopeType scope)
{
	...
	case YO_NODE_DECL_VAR:
		printf("DECL VAR "); // node->data.declVar.isMutable ? "MUT " : "");
	...
	case YO_NODE_DECL_ARG:
		printf("DECL ARG "); // node->data.declVar.isMutable ? "MUT " : "");
		yoParserDumpNode(node->data.declVar.name, depth, YO_NODE_SCOPE_OP);
		printf(" ");
		yoParserDumpNode(node->data.declVar.type, depth, YO_NODE_SCOPE_OP);
		break;
	...
	case YO_NODE_TYPE_STRUCT:
		printf("%s", node->type == YO_NODE_TYPE_STRUCT ? "STRUCT" : "CONTRACT");
...
void yoParserAssign(YYSTYPE * r, YYSTYPE * a, YYSTYPE * b, int op, void * parm, YYLTYPE * loc)
{
	...
	YO_ASSERT(op == T_ASSIGN || op == T_INIT_ASSIGN || op == T_PROP_ASSIGN
		|| op == T_PLUS_ASSIGN || op == T_MINUS_ASSIGN || op == T_MUL_ASSIGN || op == T_DIV_ASSIGN
		|| op == T_MOD_ASSIGN || op == T_OR_ASSIGN || op == T_AND_ASSIGN || op == T_XOR_ASSIGN  || op == T_POW_ASSIGN
		|| op == T_RSH_ASSIGN || op == T_LSH_ASSIGN || op == T_CONCAT_ASSIGN);
	YO_ASSERT(a->node);
	YO_ASSERT(b->node);
...
bool YoParserNode::isType() const
{
	return type == YO_NODE_TYPE_NAME || type == YO_NODE_TYPE_STD_NAME || type == YO_NODE_TYPE_MUTABLE
		...
		|| type == YO_NODE_TYPE_STRUCT || type == YO_NODE_TYPE_CONTRACT || type == YO_NODE_TYPE_FUNC
		|| type == YO_NODE_NAME;
}
...
{
	YoParser * parser = dynamic_cast<YoParser*>((YoParser*)parm);
	YO_ASSERT(name->node && (name->node->type == YO_NODE_NAME || name->node->type == YO_NODE_DOTNAME || name->node->type == YO_NODE_TYPE_ARR));
	YO_ASSERT(expr_list->node);
	YoParserNode * node = parser->newNode(YO_NODE_NEW_OBJ_EXPS, loc);
	node->data.obj.name = name->node;
...
void yoParserStruct(YYSTYPE * r, YYSTYPE * gen, YYSTYPE * extends, YYSTYPE * body, void * parm, YYLTYPE * loc)
{
	YoParser * parser = dynamic_cast<YoParser*>((YoParser*)parm);
	YO_ASSERT(gen && extends && body);
	YoParserNode * node = parser->newNode(YO_NODE_TYPE_STRUCT, loc);
	node->data.typeClass.gen = gen->node;
	node->data.typeClass.extends = extends->node;
	node->data.typeClass.body = body->node;
	r->node = node;
